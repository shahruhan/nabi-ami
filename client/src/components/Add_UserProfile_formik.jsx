...
const Add_UserProfile_page = () => {
    ...
    const handleInput = (e) =>{
        ...
        // const jsonFile = JSON.stringify(files);
        setFile(files);
        // console.log(files);
    }
    ...
    const PostUserData = async (event) => {
        // console.log(file);
        const Data = event;
		const { name, contact, gender, email, address, file,
            ...
            const filefieldname = Data.file.fieldname;
            const fileOrignalname = Data.file.orignalname;
            const fileEncoding = Data.file.encoding;
            const fileDestination = Data.file.destination;

            const filedata = { name: fileName, path: filePath, type: fileType, size: fileSize, 
                fieldname: filefieldname, orignalname: fileOrignalname, encoding: fileEncoding, destination: fileDestination}
		...
		const res = await fetch("http://localhost:5000/add_user", {
			...
			body: JSON.stringify({
                ...
                bioData, file, date, month, year, password, conPassword
			})
		});

        const fileRes = await fetch("http://localhost:5000/add_user", {
			method: "POST",
			headers: {
				"Content-Type": "application/json"
			},
                            ...
                            <div className="col-xl-12">
                                <div className="card">
                                    <div className="card-body mt-4">
                                        <Formik
                                                initialValues = {{
                                                    name: "", contact: "", gender: "", email: "",address: "", file: "",
                                                    bioData: "", profileImage: "", date: "", month: "", year: "", password: "", conPassword: ""
                                                }}
                                                    ...
                                                    <div className="col-lg-6 col-md-6">
                                                        <div className="form-group">
                                                            <label className="form-label mt-0 ms-1">Profile Image</label>
                                                                <Field id="file" className="form-control form-control-sm"
                                                                    type="file"
                                                                    name='file'
                                                                    // value={values.file}
